...
import CryptocurrencyAccordionTable from '@/components/cryptocurrency-balance-table/cryptocurrency-accordion-table';
import CryptocurrencyDrawerTable from '@/components/cryptocurrency-balance-table/cryptocurrency-drawer-table';

import { CoinBalanceData } from '@/data/static/coin-wallet-data';
...
const COLUMNS = [
...

  {
    ...
    // @ts-ignore
    Cell: ({ cell: { value } }) => (
      <div className="ltr:text-right rtl:text-right">{value}</div>
    ),
    minWidth: 80,
    maxWidth: 120,
  },
...

  {
    ...
    // @ts-ignore
    Cell: ({ cell: { value } }) => (
      <button className="ltr:text-left rtl:text-left">Send</button>
    ),
    minWidth: 80,
    maxWidth: 120,
  },
...
export default function CryptocurrencyBalanceTable() {
  // const { coins } = useCoins();
  // const data = React.useMemo(() => coins, [coins]);
  const data = React.useMemo(() => CoinBalanceData, []);
